...
// Fondo animado "Silk" (21st.dev Shader Builder) en WebGL1, sin librerías.
...
//   u_scene = vec4(canvas width, canvas height, seconds * 0.56, 4.0)
//   u_shape = vec4(1.94, 0.58, 0.48, 0.04)
//   u_surface = vec4(2.98, 1.31, 0.00, 1.00)
//   u_finish = vec4(0.00, 0.15, 0.001, 0.01)
//   u_transform = vec4(3195.0, 4.33, 0.06, 0.0)
//   u_space = vec4(-0.10, 0.02, pointer x, pointer y)
...
// #101010, #F5F5F5, #B0B0B0, #3A3A3A (+ 4 sin usar)
...
export function SilkShader({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    gl.uniform3fv(loc("u_colors"), COLORS)
    gl.uniform4f(loc("u_shape"), 1.94, 0.58, 0.48, 0.04)
    gl.uniform4f(loc("u_surface"), 2.98, 1.31, 0.0, 1.0)
    gl.uniform4f(loc("u_finish"), 0.0, 0.15, 0.001, 0.01)
    gl.uniform4f(loc("u_transform"), 3195.0, 4.33, 0.06, 0.0)
    gl.uniform4f(loc("u_space"), -0.1, 0.02, 0, 0)
    gl.uniform4f(loc("u_cursor"), 0, 2.0, 0.65, 0.46) // cursor desactivado
    ...
    const render = (now: number) => {
      ...
      gl.uniform4f(sceneLoc, canvas.width, canvas.height, seconds * 0.56, 4.0)
      gl.drawArrays(gl.TRIANGLES, 0, 3)
      frame = requestAnimationFrame(render)
    }

    // Solo anima con la pestaña visible y el canvas en pantalla
    let onScreen = true
    const sync = () => {
      const shouldRun = !document.hidden && onScreen
      if (shouldRun && !frame) frame = requestAnimationFrame(render)
      if (!shouldRun && frame) {
